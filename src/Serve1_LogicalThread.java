...
public class Serve1_LogicalThread extends Thread {
    int flag_finish;
    Socket socket_read;
    Socket socket_write;
    int Write_back_port;
    ...
    JTextField Databack;
    JTextField Databack1;
    JTextArea Databack2;
    public Serve1_LogicalThread(Socket[] socket, int port, Socket socket1, JTextField Databcak,JTextField Databcak1,JTextArea Databcak2){
    ...
    public void run(){
        ...
        //初始化流
        try {
            this.socket_read = socket[port-104];
            InputStream is=null;
    ...
    public void W_rite(int port_1,byte[] response) throws IOException //直接写回
    {
        ...
        this.socket_write = socket[port_1-104];
    ...
    public void S_Swrite(byte[] response) {
        if(this.port!=108)
        try {
    ...
    public void Send_data(byte[] response) throws IOException {//广播
        this.Write_back_port=0;
        //Socket S_S =new Socket(1000);
        S_Swrite(response);
        for(int T=104;T<108;T++)
        {
            W_rite(T,response);
        }
    ...
    private byte[] logic(byte[] b,int off,int len) throws IOException, ParseException {//创建链接
        ...
        String path="C:\\表2.txt";//读取的文件名
        File file = new File(path);
        int re1_biao=Before_Op_biao_study(Get_Sth,path);//进行查表，实现自学习
        if(re1_biao==0&&Get_Sth.Duankou!=108)//没有查到
        {
            SimpleDateFormat df = new SimpleDateFormat("yy/MM/dd-HH:mm:ss");//设置日期格式
            //System.out.println(df.format(new Date()));// new Date()为获取当前系统时间
            // String Str=String.valueOf(data.Duankou);
